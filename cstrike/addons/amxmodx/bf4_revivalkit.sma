...
static const PLUGIN_NAME	[] 		= "BF4 Revival Kit";
...
{
	{"bf4_rkit_health", 			"75",	"num"},
	{"bf4_rkit_cost",				"1200",	"num"},
	{"bf4_rkit_screen_fade",		"1",	"num"},
	{"bf4_rkit_screen_fade_time",	"2",	"num"},
	{"bf4_rkit_delay_revive",		"3",	"num"},
	{"bf4_rkit_delay_die",			"15",	"num"},
	{"bf4_rkit_deathmatch",			"1",	"num"},
	{"bf4_rkit_bot_has_kit",		"0",	"num"},
	{"bf4_rkit_bot_can_revive",		"1",	"num"},
	{"bf4_rkit_buy_mode",			"1",	"num"},
	{"bf4_rkit_buy_zone",			"1",	"num"},
	{"bf4_rkit_distance",			"75.0",	"float"},
	{"bf4_rkit_check_obstacle",		"1",	"num"},
	{"bf4_rkit_reward",				"150",	"num"},
};
...
{
	CmdBuyRKit(get_param(1));
}
public CmdBuyRKit(id)
{
	if (!g_cvars[RKIT_BUYMODE])
	{
		client_print_color(id, print_chat, "^4[BF4 Revive Kit]:^1 You can't buy in this mode. You already have a revival kit.");
		return PLUGIN_HANDLED;
	}

	if(!is_user_alive(id))
		client_print_color(id, print_chat, "^4[BF4 Revive Kit]:^1 You need to be alive.");
	else if(g_player_data[id][HAS_KIT])
		client_print_color(id, print_chat, "^4[BF4 Revive Kit]:^1 You already have a revival kit.");
	else if(!cs_get_user_buyzone(id) && g_cvars[RKIT_BUYZONE])
		client_print_color(id, print_chat, "^4[BF4 Revive Kit]:^1 You need to be in the buyzone.");
	else if(cs_get_user_money(id) < g_cvars[RKIT_COST])
		client_print_color(id, print_chat, "^4[BF4 Revive Kit]:^1 You dont have enough money (Cost:$%d)", g_cvars[RKIT_COST]);
	else
	{
		g_player_data[id][HAS_KIT] = true;
		cs_set_user_money(id, cs_get_user_money(id) - g_cvars[RKIT_COST]);
		client_print_color(id, print_chat, "^4[BF4 Revive Kit]:^1 You bought a revival kit. Hold your +use key (E) to revive a teammate.");
...
public TaskRevive(taskid)
{
	...
	{
		failed_revive(id);
		client_print_color(id, print_chat, "^4[BF4 Revive Kit]:^1 Failed target reviving.");
		remove_task(taskid);
		return PLUGIN_CONTINUE;
	}
	...
	if(g_player_data[id][REVIVE_DELAY] < get_gametime())
	{
		if(findemptyloc(body, 10.0))
		{
			...
			remove_task(taskid);
			client_print_color(id, print_chat, "^4[BF4 Revive Kit]:^1 %n revived successfully", target);
		}
	}
	return PLUGIN_CONTINUE;
}
